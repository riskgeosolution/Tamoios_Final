...
def ler_logs_eventos(id_ponto):
    try:
        ...
        if not os.path.exists(full_log_file_path): return []
        with open(full_log_file_path, 'r', encoding='utf-8') as f:
            logs_str = f.read()
        if not logs_str: return []
        logs_list = logs_str.strip().split('\n')
        if id_ponto == "GERAL": return logs_list
...
def setup_disk_paths():
    ...
    if DB_ENGINE is None:
        if is_sqlite: connect_args = {"check_same_thread": False, "timeout": 30}
        else: connect_args = {"connect_timeout": 30}
...
def initialize_database():
    ...
    try:
        with DB_ENGINE.connect() as connection: pass
        ...
        existing_indexes = []
        try: indexes = inspector.get_indexes(DB_TABLE_NAME); existing_indexes = [idx['name'] for idx in indexes]
        except Exception: pass
        with DB_ENGINE.connect() as connection:
            if 'idx_timestamp' not in existing_indexes:
                try: connection.execute(text(f'CREATE INDEX idx_timestamp ON {DB_TABLE_NAME} (timestamp)'))
                except Exception: pass
            if 'idx_id_ponto' not in existing_indexes:
                try: connection.execute(text(f'CREATE INDEX idx_id_ponto ON {DB_TABLE_NAME} (id_ponto)'))
                except Exception: pass
            connection.commit()
...
def save_to_sqlite(df_novos_dados):
    ...
    try:
        ...
        adicionar_log("DB", f"Salvando {len(df_para_salvar)} linhas no banco de dados.", level="INFO", salvar_arquivo=False)
        with DB_ENGINE.connect() as connection:
            df_para_salvar.to_sql(DB_TABLE_NAME, connection, if_exists='append', index=False)
            connection.commit()
    except Exception as e:
        if "UNIQUE constraint failed" in str(e): adicionar_log("DB", "Aviso: Dados duplicados ignorados.", level="WARN", salvar_arquivo=False)
        else: adicionar_log("DB", f"ERRO CRÍTICO Salvar DB: {e}", level="ERROR", salvar_arquivo=True)
    ...
    try:
        ...
        if 'timestamp' in df.columns and not df.empty:
            ...
            if not colunas:
                ...
                adicionar_log("DB_READ", f"Leitura: {ponto_log} | Linhas: {len(df)} | Último: {ultimo}", level="INFO", salvar_arquivo=False)
        return df
    except Exception as e:
        adicionar_log("DB", f"ERRO Leitura DB: {e}", level="ERROR", salvar_arquivo=True)
        return pd.DataFrame()

# --- CORREÇÃO: Função get_recent_data_for_worker aceita colunas ---
def get_recent_data_for_worker(hours=73, colunas=None):
    return read_data_from_sqlite(last_hours=hours, colunas=colunas)
...
def get_status_from_disk():
    try:
        ...
        with open(full_status_file_path, 'r', encoding='utf-8') as f: return json.load(f)
...
def fetch_data_from_weatherlink_api(ultimo_acumulado_chuva=None):
    ...
    with httpx.Client(timeout=30.0) as client:
        for id_ponto, config in WEATHERLINK_CONFIG.items():
            ...
            adicionar_log(id_ponto, f"Requisitando API WeatherLink (Station ID: {config['STATION_ID']}).", level="INFO", salvar_arquivo=False)
            ...
            try:
                ...
                r.raise_for_status(); response_json = r.json()
                ...
                if (int(t) - s['ts']) > 3600: adicionar_log(id_ponto, "Dados atrasados/offline.", level="WARN", salvar_arquivo=True); continue
                dados.append({"timestamp": arredondar_timestamp_10min(s['ts']), "id_ponto": id_ponto, "chuva_mm": 0.0, "precipitacao_acumulada_mm": acumulado_dia})
            except Exception as e: adicionar_log(id_ponto, f"Erro API WL: {e}", level="ERROR", salvar_arquivo=True)
...
def fetch_data_from_zentra_cloud():
    ...
    if not r or r.status_code != 200: adicionar_log(ID_PONTO_ZENTRA_KM72, f"Erro Zentra: {r.status_code if r else 'N/A'}", level="ERROR", salvar_arquivo=True); return pd.DataFrame()
    try:
        ...
        df_bloco = pd.DataFrame([{'timestamp': ts, 'id_ponto': ID_PONTO_ZENTRA_KM72, **v} for ts, v in dados_por_timestamp.items()]); df_bloco['timestamp'] = pd.to_datetime(df_bloco['timestamp'], utc=True)
        return df_bloco
    except Exception as e: adicionar_log(ID_PONTO_ZENTRA_KM72, f"Erro JSON Zentra: {e}", level="ERROR", salvar_arquivo=True); return pd.DataFrame()
