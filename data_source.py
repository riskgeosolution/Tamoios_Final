...
import warnings
import time
# ADICIONADO: delete, table, column para a correção do erro
...
def write_with_timeout(file_path, content, mode='w', timeout=15):
    ...
    if thread.is_alive():
        ...
        # Não podemos matar a thread em Python facilmente, mas podemos lançar o erro.
...
def adicionar_log(id_ponto, mensagem, level="INFO"):
    """ Adiciona uma entrada de log com nível de severidade. """
    try:
...
def ler_logs_eventos(id_ponto):
    """ Lê o log do arquivo eventos.log e retorna o conteúdo filtrado. """
    try:
...
def initialize_database():
    """ Verifica e cria a tabela e índices no SQLite. """
    global DB_ENGINE

    if DB_ENGINE is None:
        raise Exception("DB_ENGINE não inicializado. Execute setup_disk_paths() antes.")
    ...
    try:
        ...
        # Verifica índices existentes de forma segura
        existing_indexes = []
        try:
        ...
        adicionar_log("SISTEMA", f"ERRO CRÍTICO ao inicializar o banco de dados: {e}", level="ERROR")
        traceback.print_exc()
...
def save_to_sqlite(df_novos_dados):
    ...
    try:
        ...
        if "UNIQUE constraint failed" in str(e):
            adicionar_log("DB", "Aviso: Dados duplicados ignorados ao salvar.", level="WARN")
        else:
            adicionar_log("DB", f"ERRO CRÍTICO ao salvar no SQLite: {e}", level="ERROR")


def delete_from_sqlite(timestamps):
    """ Deleta linhas do banco de dados usando SQLAlchemy Expression Language (RESOLVIDO). """
    global DB_ENGINE
    if not timestamps: return

    try:
        # Formata timestamps para string ISO
        ts_strings = [pd.to_datetime(ts).strftime('%Y-%m-%d %H:%M:%S') for ts in timestamps]

        with DB_ENGINE.connect() as connection:
            # SOLUÇÃO DEFINITIVA:
            # Em vez de usar text() e bindparam() que causam conflitos de versão/driver com listas,
            # usamos a abstração 'delete' e 'table' do SQLAlchemy.
            # Isso gera o SQL correto automaticamente para qualquer lista.

            # 1. Definimos uma representação leve da tabela
            t_historico = table(DB_TABLE_NAME, column('timestamp'))

            # 2. Criamos o statement DELETE WHERE timestamp IN (...)
            stmt = delete(t_historico).where(t_historico.c.timestamp.in_(ts_strings))

            adicionar_log("DB", f"Deletando {len(ts_strings)} registros antigos para atualização (UPSERT).")
        ...
        adicionar_log("DB", f"ERRO CRÍTICO ao deletar do SQLite: {e}", level="ERROR")
        traceback.print_exc()
...
def read_data_from_sqlite(id_ponto=None, start_dt=None, end_dt=None, last_hours=None):
    ...
    try:
        ...
        adicionar_log("DB", f"ERRO CRÍTICO ao ler do SQLite: {e}", level="ERROR")
        return pd.DataFrame()
...
def get_status_from_disk():
    try:
        ...
        default_status = {
            p: {"status": "SEM DADOS", "chuva_72h": 0.0, "umidade_1m": None, "umidade_2m": None, "umidade_3m": None,
                "timestamp_local": None} for p in PONTOS_DE_ANALISE.keys()}
        adicionar_log("SISTEMA", f"Criando status padrão (arquivo novo ou erro).", level="WARN")
        return default_status


# --- INTEGRAÇÃO WEATHERLINK ---
    ...
    with httpx.Client(timeout=30.0) as client:
        for id_ponto, config in WEATHERLINK_CONFIG.items():
            if "SUA_CHAVE" in config.get('API_KEY', ''):
                adicionar_log(id_ponto, "Chave API não configurada.", level="WARN")
                continue
            ...
            try:
                ...
                s = next((s['data'][0] for s in response_json.get('sensors', [])
                          if (s.get('data_structure_type') == 10 or s.get('sensor_type') == 48) and s.get('data')),
                         None)

                if not s or 'ts' not in s or (int(t) - s['ts']) > 3600:
                    adicionar_log(id_ponto, "Dados atrasados ou estação offline.", level="WARN")
                    continue

                chuva_15 = s.get('rainfall_last_15_min_mm', 0.0) or 0.0
                dados.append({
                ...
                adicionar_log(id_ponto, f"Erro API WeatherLink: {e}", level="ERROR")
...
def fetch_data_from_zentra_cloud():
    ...
    if not r or r.status_code != 200:
        adicionar_log(ID_PONTO_ZENTRA_KM72, f"Erro na API Zentra: Status {r.status_code if r else 'N/A'}",
                      level="ERROR")
        return pd.DataFrame()
    ...
    try:
            ...
            if port_num in MAPA_ZENTRA_KM72:
                ...
                for reading in sensor_block.get('readings', []):
                    ...
                    if ts_iso and value is not None:
                        ts_arredondado = arredondar_timestamp_10min(datetime.datetime.fromisoformat(ts_iso).timestamp())
        ...
        adicionar_log(ID_PONTO_ZENTRA_KM72, f"Erro JSON Zentra: {e}", level="ERROR")
        return pd.DataFrame()
        ...
        if response and response.status_code == 200:
            try:
                ...
                if wc_data:
                    ...
                    if dados:
                        df = pd.DataFrame([{'timestamp': k, 'id_ponto': id_ponto, **v} for k, v in dados.items()])
                        df['timestamp'] = pd.to_datetime(df['timestamp'], utc=True)
                        save_to_sqlite(df)
            except Exception:
                pass
...
def backfill_weatherlink_data(id_ponto):
    ...
    adicionar_log(id_ponto, "Iniciando Backfill WeatherLink.")
                    ...
                    for s in data.get('sensors', []):
                        if s.get('sensor_type') == 48 or s.get('data_structure_type') == 10:
                            for r in s.get('data', []):
                                if 'rainfall_mm' in r:
                                    dados.append({
                                        "timestamp": arredondar_timestamp_10min(r['ts']),
                                        "id_ponto": id_ponto,
