...
if __name__ == '__main__':
    ...
    if len(args) > 1 and args[1].lower() == 'backfill':
        if len(args) != 4:
            ...
            print("IDs disponíveis:", ", ".join(PONTOS_DE_ANALISE.keys()))
        else:
            id_ponto_backfill = args[2]
            try:
                dias = int(args[3])
                if id_ponto_backfill not in PONTOS_DE_ANALISE:
                    print(f"ID de ponto inválido: {id_ponto_backfill}. Use um dos IDs disponíveis.")
                else:
                    print(f"--- MODO DE MANUTENÇÃO: BACKFILL PARA {id_ponto_backfill} NOS ÚLTIMOS {dias} DIAS ---")
                    data_source.backfill_weatherlink_data_manually(id_ponto_backfill, dias)
                    print("--- BACKFILL CONCLUÍDO ---")
            except ValueError:
                print("O número de dias deve ser um inteiro.")
            except Exception as e:
